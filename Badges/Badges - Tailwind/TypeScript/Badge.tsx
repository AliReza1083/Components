...
const Badge = ({ technology }: BadgeProps) => {
  const finding = BADGES.find((badge) => badge.inputName == technology);
  return (
    <button
      style={{ borderColor: `#${finding?.color}` }}
      className="flex items-center gap-2 py-1 px-2 rounded-[4px] text-base border-2 font-medium"
    >
      <span className="text-xl" style={{ color: `#${finding?.color}` }}>
        {finding?.icon}
      </span>
      {finding?.technology}
    </button>
